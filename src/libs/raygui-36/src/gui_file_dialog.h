...
void GuiFileDialog(GuiFileDialogState *state)
{
    if (state->windowActive)
    {
        ...
        {
            dirFilesIcon = (FileInfo *)calloc(MAX_DIRECTORY_FILES, sizeof(FileInfo));    // Max files to read
            for (int i = 0; i < MAX_DIRECTORY_FILES; i++) dirFilesIcon[i] = (char *)calloc(MAX_ICON_PATH_LENGTH, 1);    // Max file name length
        }
...
# endif
        ...
        if (!state->windowActive)
        {
            ...
            for (int i = 0; i < MAX_DIRECTORY_FILES; i++) free(dirFilesIcon[i]);
